...
<div class="group-box">
	<div align="center">
	<div class="title">Nghành đào tạo</div>
	...
	?>
		<form method="post" name="" action="<?php echo $_SERVER["PHP_SELF"];?>">			
			...
			<?php 
			// tính tổng số dòng 
			$sql = "SELECT COUNT(*) FROM dbo_nganh ";
			$result = $db->query($sql);
			$total_row = 0;
			if ($result){
			...
			$sql = "SELECT * FROM dbo_nganh  n join dbo_khoa  k on k.makhoa=n.makhoa order by manganh LIMIT $start_row, $limit ";
			$result = $db->query($sql);			 
		...
		<table class="ds">
			<!-- in tiêu đề danh sách -->
				...
				<tr>
					...
					<th>STT</th>
					<th  style="width:70px">Mã Ngành</th>
					<th  style="width:200px">Tên Ngành </th>
