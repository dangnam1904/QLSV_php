...
<div class="group-box">
	...
	?>
		<form method="post" name="frmSV" action="<?php echo $_SERVER["PHP_SELF"];?>">			
			...
			<?php 
			// tính tổng số dòng 
			$sql = "SELECT COUNT(*) FROM dbo_khoa ";
			$result = $db->query($sql);
			$total_row = 0;
			if ($result){
			...
			$sql = "SELECT * FROM dbo_khoa  order by makhoa LIMIT $start_row, $limit ";
			$result = $db->query($sql);			 
		...
		<table class="ds">
			<!-- in tiêu đề danh sách -->
				...
				<tr>
					...
					<th  style="width:400px">Tên Khoa </th>
